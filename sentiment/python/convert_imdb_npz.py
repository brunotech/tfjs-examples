...
if __name__ == '__main__':
  ...
  for split in ('train', 'test'):
    data_path = f'{args.out_prefix}_{split}_data.bin'
    xs = x_train if split == 'train' else x_test
    print(f'Writing data to file: {data_path}')
    with open(data_path, 'wb') as f:
      for sentence in xs:
        f.write(struct.pack('%di' % len(sentence), *sentence))

    targets_path = f'{args.out_prefix}_{split}_targets.bin'
    ys = y_train if split == 'train' else y_test
    print(f'Writing targets to file: {targets_path}')
